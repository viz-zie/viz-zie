...
FONT_PATH = os.environ.get("WORDMARK_FONT", "/System/Library/Fonts/Avenir Next.ttc")
FONT_INDEX = int(os.environ.get("WORDMARK_FONT_INDEX", 0))   # face within a .ttc (Bold)
# two letters across the full width leaves generous grid columns each, which is what
# lets the cells be big enough to read as characters rather than as dither.
TEXT = os.environ.get("WORDMARK_TEXT", "VK")
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">viz-zie@github: ~$ ./wordmark.sh --3d</text>')
